    ...
    <SafeAreaView style={{ flex: 1 }}>
      <View style={styles.container}>
        <TouchableOpacity onPress={handleNavigateBack}>
          <Icon name="log-out" size={24} color="#34cb79" style={styles.icon} />
        </TouchableOpacity>
          ...
          { initalPosition[0] !== 0 && (
            ...
            >
              {points.map(point => (
                ...
                >  
                  <View style={styles.mapMarkerContainerAll}>
      ...
      <View style={styles.itemsContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20 }}>
          {items.map(item => (
            <TouchableOpacity style={[styles.item, selectedItems.includes(item.id) ? styles.selectedItem : {} ]} onPress={() => { handleSelectedItem(item.id) }} key={String(item.id)} activeOpacity={0.6}>
